"""
Generate publication figures for the TALH exploratory ablation paper.
...
def fig_ttft_scaling():
    ...
    # Highlight the flattest observed curve without assigning a scaling law.
    mla_vals = data["mla_only"]
    ax.fill_between(contexts, mla_vals, alpha=0.08, color=COLORS["mla_only"])
    ax.text(1900, mla_vals[1] - 400, "Flattest observed\ncurve", color=COLORS["mla_only"],
            fontsize=8.5, ha="center", style="italic")

    ax.set_xlabel("Prompt length (tokens)")
    ax.set_ylabel("Time to first token — TTFT (ms)")
    ax.set_title("Preliminary TTFT on M3 MacBook\n(no repeated-trial records)")
...
# ── Figure 5: Phase-1 step/token-matched chart (all 5 variants, step 4K) ─────
def fig_phase1_matched():
    """
    All five variants trained for the same 4,000 steps and nominal tokens with
    the same 256→512 context curriculum. Parameters and operations are unmatched.
    ...
    ax.set_xticklabels([LABELS[v] for v in variants], rotation=15, ha="right")
    ax.set_ylabel("Validation Perplexity  (lower = better)")
    ax.set_title("Step- and Token-Matched Quality Comparison\n"
                 "All 5 variants — size and operations are unmatched")
    ax.yaxis.grid(True, linestyle="--", alpha=0.4)
    ax.set_axisbelow(True)
    ax.set_ylim(0, 370)
...
def fig_all_variants_phase1():
    ...
    ax.set_xlabel("Training step")
    ax.set_ylabel("Validation loss  (lower = better)")
    ax.set_title("Phase-1 Training: All Variants (Same Steps and Tokens)")
...
def fig_pareto_matched():
    """Quality/memory plot using step- and token-matched Phase-1 PPL."""
    pts = {
    ...
    ax.set_xlabel("Peak GPU memory during training (GB)")
    ax.set_ylabel("Validation perplexity at 512 tokens\n(step 4K — lower = better)")
    ax.set_title("Quality vs. Memory — Same Steps and Nominal Tokens")
    ax.yaxis.grid(True, linestyle="--", alpha=0.4)
    ax.xaxis.grid(True, linestyle="--", alpha=0.4)
    ax.set_axisbelow(True)
    ax.set_ylim(215, 325)

    ax.annotate("Lowest measured PPL +\nlowest hybrid memory",
                xy=(24.01, 231.1), xytext=(16, 219),
                arrowprops=dict(arrowstyle="->", color="#2196F3", lw=1.5),
                color="#2196F3", fontsize=8.5, ha="center",
                bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="#2196F3", lw=1))
